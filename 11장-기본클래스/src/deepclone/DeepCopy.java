...
class Rectangle implements Cloneable {
	 ...
	 */
	//clone 메서드 본인(=부모=Object) 타입으로 재정의 ->이것 사용시 메인클래스에서 복제할 때 강제로 형변환 해야함 
	/*
	@Override
	public Object clone() throws CloneNotSupportedException {
		//1.먼저 얕은 복제로 객체를 통째로 복사한다.
		Rectangle cpy = (Rectangle)super.clone();
		//2.객체가 참조하는 또다른 객체까지 복사한다.=> upperLeft, lowerRight가 참조하는 Point 객체를 복사한다.
	...
	*/
	...
	//clone 메서드 자식클래스 타입으로 재정의 -> 이것 사용시 메인클래스에서 복제할 때 강제 형변환 안해도 자동 형변환 됨
	@Override
	public Rectangle clone() throws CloneNotSupportedException {
		//1.먼저 얕은 복제로 객체를 통째로 복사한다.
		Rectangle cpy = (Rectangle)super.clone();
		//2.객체가 참조하는 또다른 객체까지 복사한다.=> upperLeft, lowerRight가 참조하는 Point 객체를 복사한다.
//		cpy.upperLeft = (Point)upperLeft.clone();//부모타입 그대로 재정의하면 강제형변환 해야함
//		cpy.lowerRight = (Point)lowerRight.clone();
		cpy.upperLeft = upperLeft.clone();//자식클래스타입으로 재정의하면 강제형변환 안해도됨
		cpy.lowerRight = lowerRight.clone();
		//3.깊은 복제된 cpy를 리턴한다.
		return cpy;
	}
}
...
public class DeepCopy {
	...
	public static void main(String[] args) {
		...
		System.out.println("======원본의 값=======");
		org.showPosition();//원본의 값
		Rectangle cpy;
		try {
			//if (org instanceof Rectangle) {//오버라이딩 할 때 자식클래스 타입으로 변경해서 if절 사용안해도된다.
				//cpy = (Rectangle)org.clone();//강제 형변환의 번거로움을 없애주어 안해도됨
				cpy = org.clone();
				System.out.println("======복제의 값======");
				cpy.showPosition();//복제의 값
				
				System.out.println("**********원본 값 변경 후 값 확인**********\n");
				
				org.changePos(2, 2, 7, 7);//원본 값 변경
				System.out.println("======원본의 값======");
				org.showPosition();//원본 값은 변경됨
				System.out.println("======복제의 값======");
				cpy.showPosition();//원본 값 변경 후 복제 값 확인 => 원본 값 변경 시 복제 값도 변경안됨
				
				System.out.println("**********복제 값 변경 후 값 확인**********\n");
				
				cpy.changePos(3, 3, 5, 5);//복제 값 변경
				System.out.println("======원본의 값======");
				org.showPosition();//복제 값 변경 후 원본 값 확인 => 복제 값 변경 시 원본 값도 변경안됨
				System.out.println("======복제의 값======");
				cpy.showPosition();//복제 값은 변경됨
			//}
